...
curl --header "Content-Type: application/json-patch+json"  --request PATCH  --data '\[{"op": "add", "path": "/status/capacity/pod.alpha.kubernetes.io~1opaque-int-resource-foo", "value": "5"}\]'  [http://k8s-master:8080/api/v1/nodes/k8s-node-1/status](http://k8s-master:8080/api/v1/nodes/k8s-node-1/status)
