...
Go 客户端可以使用与 kubectl 命令行工具相同的 [kubeconfig 文件](https://kubernetes.io/docs/concepts/cluster-administration/authenticate-across-clusters-kubeconfig) 来定位和验证 apiserver。参考官方 [示例](https://git.k8s.io/client-go/examples/out-of-cluster-client-configuration/main.go) 和 [client-go 示例](../../kai-fa-zhi-nan/client-go-sample.md)。
...
`http://`_kubernetes\_master\_address_`/api/v1/namespaces/`_namespace\_name_`/services/`_service\_name\[:port\_name\]_`/proxy`

如果您没有指定 port 的名字，那么您不必在 URL 里指定 port\_name。
