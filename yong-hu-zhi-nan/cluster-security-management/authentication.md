...
![Kubernetes OpenID Connect Flow](../../.gitbook/assets/kubernetes-oidc-login.jpg)
...
| 参数 | 描述 | 示例 | 是否必需 |
| :--- | :--- | :--- | :--- |
| `--oidc-issuer-url` | 允许 API server 发现公共签名密钥的提供者的 URL。只接受使用 `https://` 的方案。通常是提供商的 URL 地址，不包含路径，例如“[https://accounts.google.com](https://accounts.google.com)” 或者 “[https://login.salesforce.com](https://login.salesforce.com)”。这个 URL 应该指向下面的 .well-known/openid-configuration | 如果发现 URL 是 `https://accounts.google.com/.well-known/openid-configuration`，值应该是`https://accounts.google.com` | 是 |
...
如果为 `--oidc-username-claim` 选择了除 `email` 以外的其他声明，则该值将以 `--oidc-issuer-url` 作为前缀，以防止与现有 Kubernetes 名称（例如 `system:users`）冲突。例如，如果提供商网址是 [https://accounts.google.com，而用户名声明映射到](https://accounts.google.com，而用户名声明映射到) `jane`，则插件会将用户身份验证为：
...
您可以使用 `certificates.k8s.io` API将 x509 证书配置为用于身份验证，如 [此处](https://kubernetes.io/docs/tasks/tls/managing-tls-in-a-cluster) 所述。 官方最新文档地址：[https://kubernetes.io/docs/admin/authentication/](https://kubernetes.io/docs/admin/authentication/)
